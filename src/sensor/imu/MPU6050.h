...
int mpu_enable_aux_master(void);
int mpu_aux_read_reg(uint8_t dev7, uint8_t reg, uint8_t *val);
int mpu_aux_write(uint8_t dev7, uint8_t reg, uint8_t val);
int mpu_aux_stream_mag_start(void);
int mpu_aux_read_mag6(uint8_t raw[6]);
int mpu_aux_slv0_enable(bool en);
int mpu_aux_set_clock_slow(bool slow);
...
int mpu_set_bypass(bool en);
void mpu_fifo_reset(void);

int mpu_ext_passthrough(bool enable);

extern const sensor_imu_t sensor_imu_mpu6050;
