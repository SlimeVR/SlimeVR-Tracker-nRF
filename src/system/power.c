...
static uint32_t *dbl_reset_mem __attribute__((unused)) =
        ((uint32_t *)DFU_DBL_RESET_MEM); // retained
...
static int16_t last_pptt[BATTERY_SAMPLES - 1] = {[0 ... BATTERY_SAMPLES - 2] =
                                                         -1};
...
K_THREAD_DEFINE(disable_DFU_thread_id,
                128,
                disable_DFU_thread,
                NULL,
                NULL,
                NULL,
                DISABLE_DFU_THREAD_PRIORITY,
                0,
                500); // disable DFU if the system is running correctly

static void power_thread(void);
K_THREAD_DEFINE(power_thread_id,
                1024,
                power_thread,
                NULL,
                NULL,
...
static const struct gpio_dt_spec ldo_en =
        GPIO_DT_SPEC_GET(ZEPHYR_USER_NODE, ldo_gpios);
#else
#pragma message "LDO enable GPIO does not exist"
#endif
#if DT_NODE_HAS_PROP(ZEPHYR_USER_NODE, chg_en_gpios)
#define CHG_EN_EXISTS true
static const struct gpio_dt_spec chg_en =
        GPIO_DT_SPEC_GET(ZEPHYR_USER_NODE, chg_en_gpios);
#else
...
static void sys_disconnect_interface_pins(void) {
    // interface pins are disconnected according to devicetree, so only need to
    // disconnect any cs pins int pin already configured by power off
...
#endif
        /*
                TODO: for promicro, leaving ext_vcc on draws ~50uA, disconnect
           works, pulldown may be more reliable what to do about boards that use
           ext_vcc? it is not expected to leave on during WOM
        */
}

void sys_interface_suspend(void) {
#if DT_NODE_HAS_STATUS_OKAY(DT_PARENT(DT_NODELABEL(imu_spi)))
    const struct device *const pm_spi_imu =
    ...
    const struct device *const pm_i2c_imu =
    ...
    const struct device *const pm_spi_mag =
    ...
    const struct device *const pm_i2c_mag =
...
// TODO: the gpio sense is weird, maybe the device will turn back on immediately
// after shutdown or after (attempting to) enter WOM
// TODO: there should be a better system of how to handle all system_off cases
// and all the sense pins
...
// TODO: on some boards there is actual power path, try to use the LED in this
// case
// TODO: usually charging, i would flash LED but that will drain the battery
...
static void sys_WOM(bool force) // TODO: if IMU interrupt does not exist what
                                // does the system do?
{
    LOG_INF("IMU wake up requested");
#if IMU_INT_EXISTS
    if (CONFIG_0_SETTINGS_READ(CONFIG_0_DELAY_SLEEP_ON_STATUS) && !force
        && (!esb_ready()
            || !status_ready())) // Wait for esb to pair in case the user is
                                 // still trying to pair the device
    {
        if (!system_off_timeout)
            system_off_timeout =
                    k_uptime_get() + 30000; // allow system off after 30 seconds
                                            // if status errors are still active
        ...
        // TODO: this may mean the system never enters system off if
        // sys_request_WOM is not called again after the timeout
    }
    ...
    //	retained_update();
    wait_for_logging();
#if ADAFRUIT_BOOTLOADER // if using Adafruit bootloader, always skip dfu for
                        // next boot
...
#endif
    // Disconnect remaining interface pins // TODO: only an improvement during
    // shutdown? causes higher usage in WOM
...
static void update_battery(int16_t battery_pptt) {
    // Plugged state will cause a sudden change in SOC >10%, so reset the sample
    // array
    if (average_pptt >= 0 && NRFX_ABS(battery_pptt - average_pptt) > 1000) {
        LOG_INF("Change to battery SOC: %5.2f%% -> %5.2f%%",
                (double)average_pptt / 100.0,
                (double)battery_pptt / 100.0);
    ...
    for (uint8_t i = BATTERY_SAMPLES - (samples - samples / 8);
         i < (BATTERY_SAMPLES - samples / 8);
         i++) {
        if (sorted_pptt[i] != -1) {
...
// TODO: this thread is handling reading charging state, battery state, dock
// state, and setting status/led
...
static void power_thread(void) {
    while (1) {
        ...
        case 0:
            ...
            if (!chg_temp_warn
                && (temp < 5.f
                    || temp > 45.f)) // this is still safe (hard limit is 0C,
                                     // but that is dangerous)
                chg_temp_warn = true;
            else if (chg_temp_warn && temp > 10.f
                     && temp < 40.f) // safest range
                chg_temp_warn = false;
        default:
            break;
        }
        chg_ret = set_charger_enable(!chg_temp_warn, device_plugged);
        // chg_ret = -1: outside safe temp range, but charger could not be
        // disabled chg_ret = 0 and chg_temp_warn = true: out of temp range,
        // charger is disabled or already has thermistor
        ...
        bool battery_available =
                battery_mV > 1500
                && !abnormal_reading; // Keep working without the battery
                                      // connected, otherwise it is obviously
                                      // too dead to boot system
        bool battery_discharged =
                battery_available
        ...
        if (!device_plugged
            && (charging || charged || plugged || usb_plugged)) {
            device_plugged = true;
        ...
        device_charged = charged; // TODO: timer on device_plugged could be used
                                  // to infer charged state
        ...
        bool runtime_low =
                k_ticks_to_ms_floor64(runtime) < CONFIG_3_SETTINGS_READ(
                        CONFIG_3_BATTERY_LOW_RUNTIME_THRESHOLD);
        bool pptt_low = current_battery_pptt < 1000;
        if (battery_available && !battery_low
            && (runtime_valid ? runtime_low : pptt_low))
            battery_low = true;
        else if (!battery_available
                 || (battery_low
                     && (runtime_valid
                                 ? !runtime_low
                                 : !pptt_low))) // hysteresis already provided
            battery_low = false;

        sys_update_battery_tracker_voltage(battery_mV, device_plugged);
        if (samples == BATTERY_SAMPLES || device_plugged)
            sys_update_battery_tracker(current_battery_pptt, device_plugged);
        calibrated_battery_pptt =
                sys_get_calibrated_battery_pptt(current_battery_pptt);

        connection_update_battery(battery_available,
                                  device_plugged,
                                  device_charged,
                                  calibrated_battery_pptt,
                                  battery_mV);
        ...
        if (chg_temp_warn && plugged) // don't need to warn if not plugged in
            set_led(SYS_LED_PATTERN_WARNING,
                    SYS_LED_PRIORITY_SYSTEM); // not critical
        else if (charging)
